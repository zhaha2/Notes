...
#### 总结
只有this才判断上下文，谁掉用的。普通的变量不用判断，直接判断作用域链就行（this第七道）（只有出现this这个东西才考虑this，其他的变量a就是自己，不是this.a）

**this不会传承，继承下去**，函数中的函数的this不是继承前面的this，而是里面的函数写在哪，谁调用就指谁（arguments调用就指向arguments）
