...
注意：a 是一个闭包，其中的x引用的是闭包变量，开始的时候为2.
a.y()改变的也是闭包中的变量
...
注意这里`c: b`**不是循环引用**，因为给c赋值的时候b因为var变量提升还是undefined。
...
作用域是指函数内部，这里的箭头函数，也就是sayHello，**所在的作用域其实是最外层的js环境**，因为**没有其他函数包裹**；然后最外层的js环境指向的对象是winodw对象，所以这里的this指向的是window对象。
...
注意箭头函数继承外面函数的this，所以先判断外面函数的this
...
promise reject了就不处理其他的promise回调了，但是其他promise已经发送了就还是会执行
...
坑：这里需要注意的是在async1中await后面的Promise是没有返回值的（没resolve），也就是它的状态始终是pending状态，所以在await之后的内容是不会执行的，包括async1后面的 .then。
...
注意函数内的**形参和外面的实参不是一个东西**。
- 如果实参是基础值的话，形参就是他的拷贝（之间不影响，没有任何关系哦）
