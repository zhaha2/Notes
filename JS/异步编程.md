...
#### new Promise() 是同步代码
  promise函数体内的resolove();前面和后面的代码都是同步的，都会同步执行
...
```
then不是微任务吗，执行时候p2还是pending，应该不调用。还是说是下轮事件循环又调用了。
...
他和then一样，都会**改变**promise的状态。相当于改变指针，让指针前进一步。
...
>如果作为参数的 Promise 实例，自己定义了catch方法，那么它一旦被rejected，**并不会触发**Promise.all()的catch方法。
